...
import model.column.ColumnStringTest;
import model.dataset.Dataset;
import model.point.IPoint;
import model.point.Passenger;

public class DistanceEuclidiennePassengerTest {
    ...
    public void should_get_distance_passenger_not_normalised(){
        distance = new DistancePassenger(false,DistanceAlgorithm.EUCLIDEAN, classColumn, ageColumn, sibspColumn, parchColumn, ticketPriceColumn, sexColumn ,cabinColumn, portEmbarkationColumn);
        
        double distanceExpected = Math.sqrt(
        ...
        assertEquals(distanceExpected, distance.distanceEuclideanNotNormalized(passenger1, passenger2));

        distanceExpected = Math.sqrt(
        ...
        assertEquals(distanceExpected, distance.distanceEuclideanNotNormalized(passenger1, passenger3));
        ...
        assertEquals(distanceExpected, distance.distanceEuclideanNotNormalized(passenger2, passenger3));
    }
    ...
    public void should_get_passenger_distance_normalised(){
        ...
        assertEquals(distanceExpected, distance.distanceEuclideanNormalized(passenger1, passenger2));
    }
    ...
    protected double calculDistanceNotNormalizedAttributeIsolated(Passenger p1, Passenger p2, ColumnNumber column){
        ...
        if(distance.isNormalized.getValue())  return Math.pow(Math.pow(value1.doubleValue() - value2.doubleValue(), 2), column.getType().getCoefficientForDistanceNormalized());
        
        return Math.pow(Math.pow(value1.doubleValue() - value2.doubleValue(), 2), column.getType().getCoefficientForDistanceNotNormalized());
    }

    protected double calculDistanceNormalizedAttributeIsolated(Passenger p1, Passenger p2, ColumnNumber column){
        if(distance.isNormalized.getValue()) return Math.pow(Math.pow((double)p1.getNormalizedValue(column) - (double)p2.getNormalizedValue(column), 2), column.getType().getCoefficientForDistanceNormalized());

        return Math.pow(Math.pow((double)p1.getNormalizedValue(column) - (double)p2.getNormalizedValue(column), 2), column.getType().getCoefficientForDistanceNotNormalized());
    }
